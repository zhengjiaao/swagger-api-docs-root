...
//    String location = "/src/main/resources/swagger2/petstore.json";
...
//    String location = "http://127.0.0.1:8088/v2/api-docs";
        ...
        Swagger2ToWordByPoiTl swagger = new Swagger2ToWordByPoiTl();
        swagger.toWord(location, "/templates/word/swagger-1.docx", null, null);
    }
    ...
    /**
     * 支持本地路径、远程url
     * @param location  本地json文件 or 远程url
     * @param templatePath 资源下模板路径
     */
    public void toWord(String location, String templatePath, File templateFile, HttpServletResponse response) throws IOException {
        SwaggerParser swaggerParser = new SwaggerParser();
        Swagger swagger = swaggerParser.read(location);
        SwaggerView viewData = this.convert(swagger);
        ...
        if (templateFile != null) {
            //本地模板
            xwpfTemplate = XWPFTemplate.compile(templateFile, config)
                    .render(viewData);
        } else {
            //资源下的模板
            xwpfTemplate = XWPFTemplate.compile(this.getClass().getResourceAsStream(templatePath), config)
                    .render(viewData);
        }
        ...
        if (response != null) {
            ...
            //写入本地
            xwpfTemplate.writeToFile("swagger-api-word" + ".docx");
        }
    }

    @SuppressWarnings("rawtypes")
    public SwaggerView convert(Swagger swagger) {
...
//                System.out.println(name);
                ...
                if (null != model.getProperties()) {
                    List<com.zja.swagger.poitl.Property> properties = new ArrayList<>();
                    model.getProperties().forEach((key, prop) -> {
                        com.zja.swagger.poitl.Property property = new com.zja.swagger.poitl.Property();
